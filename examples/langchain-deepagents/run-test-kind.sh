...
# Use a script-owned namespace so cleanup cannot touch unrelated sandboxes
# in the `default` namespace (or any shared namespace).
NAMESPACE="${LANGCHAIN_E2E_NAMESPACE:-langchain-deepagents-e2e}"
...
cleanup() {
    ...
    # Delete the whole script-owned namespace — cheaper and safer than
    # `kubectl delete sandboxclaim --all`, which would nuke unrelated
    # sandboxes if the user pointed NAMESPACE at a shared namespace.
    if [[ "${NAMESPACE}" == "default" || "${NAMESPACE}" == "kube-system" ]]; then
        echo "Refusing to delete resources in shared namespace '${NAMESPACE}'."
    else
        kubectl delete namespace "${NAMESPACE}" --ignore-not-found --wait=false 2>/dev/null || true
    fi

    # The SandboxTemplate is cluster-scoped-ish (namespaced, but deployed
    # into the same script-owned namespace). It's removed with the namespace
    # above; this is a belt-and-braces no-op if cleanup runs twice.
    kubectl delete -f "${SCRIPT_DIR}/sandbox-template.yaml" -n "${NAMESPACE}" --ignore-not-found 2>/dev/null || true
...
kubectl apply -n "${NAMESPACE}" -f sandbox-template.yaml
